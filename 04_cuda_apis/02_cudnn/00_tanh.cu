...
int main() {
    ...
    const int width = 224;
    const int tensor_size = batch_size * channels * height * width; // This is like 1.6 GB worth of data.
    ...
    // CUDNN_TENSOR_NCHW -> batch size * channels * height  * width
    ...
    std::chrono::duration<double> diff_cpu_time = end_cpu - start_cpu;
    ...
    printf("Average CPU activation time: %.3f ms\n", diff_cpu_time.count());
...
// Outputs:
// ~/Git/CudaExploration/04_cuda_apis/02_cudnn$ nvcc -lcudnn  -o ./00_exec ./00_tanh.cu 
// ~/Git/CudaExploration/04_cuda_apis/02_cudnn$ ./00_exec 
// Tensor size: 256 x 32 x 224 x 224
// Average CPU activation time: 8.739 ms
// Average Naive CUDA kernel time: 20.459 ms
// Average cuDNN activation time: 222.853 ms
// Speedup: 0.09x
